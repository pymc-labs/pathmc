...
Because the result knows the outcome, the accessors take no arguments:

```{python}
print(f"Causal ATE (do-operator): {ate.mean():.3f}  (true = {TRUE_EFFECT})")
...
print(f"Doing   (do-operator): {ate_rand.mean():.3f}")
...
```{python}
ate = model.ate("Y", "X", values=(0.0, 1.0))
print(f"ATE of X on Y: {ate.mean():.3f}")
print(f"94% HDI:       {ate.hdi(prob=0.94)}")
```
