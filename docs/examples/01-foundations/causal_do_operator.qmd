...
print(f"Seeing  (OLS slope): {seeing_slope:.3f}   True effect: {TRUE_EFFECT}")
```
...
print(f"ATE (no cond):  {ate.mean():.3f}")
```
...
print(f"{'ATU':<12} {atu.mean():>10.3f} {truth['true_atu']:>10.3f}")
```
...
Always pair `do()` with careful thinking about your causal assumptions. See [Causal Identification](/docs/examples/identification/causal_identification.qmd) for how to verify that your model identifies the effect you want, including what to do when confounders are unobserved.
...
- **The do() API** provides `.ate()`, `.cate()`, `.att()`, and `.atu()` for common causal queries — each returning an `EstimandResult` whose `.mean()`, `.hdi()`, and `.prob()` default to the outcome — plus raw `do()` and `DoResult` subtraction for custom contrasts.
