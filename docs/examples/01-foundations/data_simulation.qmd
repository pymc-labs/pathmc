...
Most foundational examples in these docs build their datasets with explicit NumPy array code that anyone can audit. But pathmc can also generate data directly from a specified model, guaranteeing that the simulated dataset is perfectly consistent with the model's generative structure.
...
:::: {.callout-note}
## MMM examples run on `simulate()`

The applied-models notebooks are moving to `pathmc.simulate()` for their ground-truth DGPs: [MMM with Latent Brand Awareness](/examples/05-applied-models/mmm_awareness.qmd) simulates a latent AR(1) mediator via the `latent=` and `families=` arguments, and pairs with [Media Mix Models](/examples/05-applied-models/mmm.qmd), which walks through panel adstock/saturation structures under partial pooling. Everything shown on this page extends to those settings through the same `params` interface.
::::
...
`pathmc.simulate_params_template()` replaces this manual step: it compiles the same zero-filled placeholder generative model that `simulate()` builds internally and returns every required parameter name with its expected shape and dtype — without drawing any data.
...
for name, desc in sorted(template.items()):
    print(f"{name:>8}: {desc['kind']:>6} {str(desc['shape']):>5} ({desc['dtype']})")
```

Each value describes how to fill `params`: `"kind"` is `"scalar"`, `"vector"`, or `"matrix"`, `"shape"` lists the axis lengths, and `"dtype"` gives the NumPy dtype. Panel models accept `panel=` and `pooling=` exactly as `model()` does; under `pooling="partial"`, unit-indexed hierarchical parameters are reported with concrete shapes derived from the placeholder data (e.g. `alpha_{var}` has one entry per panel unit), and transform terms contribute their named hyperparameters (e.g. the adstock `decay=theta_tv` argument appears as `theta_tv`).
...
- **`pathmc.simulate_params_template()`** lists every parameter name, shape, and dtype a spec requires — including hierarchical, transform, residual-covariance, and hsgp parameters — so `params` can be authored without trial and error.
