...
We generate a national-level time series of 50 weeks with a known DGP so we can verify the model recovers the true parameters. Instead of hand-writing the AR(1) recursion with NumPy, we generate the dataset with `pathmc.simulate()`: awareness is declared `latent` with the `"latent_normal"` family, and `init_awareness` seeds the scan carry at week 1.
...
exog = pd.DataFrame({
    ...
    "upper_funnel": rng.uniform(5, 25, size=n_weeks),
    "lower_funnel": rng.uniform(5, 20, size=n_weeks),
})
...
df = pathmc.simulate(
    ...
    data=exog,
    params={
        "beta_awareness": [0.0, true_a, true_rho],
        # zero process noise keeps awareness deterministic, as in Part 1
        "sigma_awareness": 0.0,
        "init_awareness": [0.0],
        "beta_sales": [0.0, true_b, true_c, true_d],
        "sigma_sales": true_sigma,
    },
...
The simulated DataFrame now carries the latent `awareness` trajectory alongside sales — ground truth we would never observe in reality, which makes it easy to eyeball what the model should recover later.

:::: {.callout-note collapse="true"}
## Why `"latent_normal"` with zero process noise?

Part 1 treats awareness as an exact function of its parents (deterministic), while Part 2 adds process noise. The `"latent_normal"` family is what gives a latent variable its own `sigma` and `init_` parameters in `simulate()`; setting `sigma_awareness = 0.0` recovers the deterministic recursion of this part while exercising the same API used throughout.
::::
...
### The attribution trap: why naive regression undervalues upper-funnel
...
See the [Panel Data Models](../panel-time/panel_data.qmd) example for a simpler introduction to `lag()` dynamics, or the [Media Mix Models](mmm.qmd) notebook for transforms, mediation, and hierarchical structures.
:::
