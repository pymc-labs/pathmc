...
The verdict has two parts. **Informative** means few of the 200 random relabelings share the DAG's Markov equivalence class — so the arrow directions are characteristic enough to be testable. **Beats the baseline** means the DAG violates fewer Local Markov Conditions than almost all of the shuffled competitors. Both hold, so the DAG is *not rejected*.
...
- `p_value_lmc` — the fraction of permutations whose LMC-violation rate is no worse than the given DAG's. The DAG is **falsified** (rejected) only when it is informative *and* `p_value_lmc > significance_level` (it fails to beat the baseline).
