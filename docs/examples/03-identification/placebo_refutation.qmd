...
This page assumes you have read [Treatment Effects with Non-Linear Models](/docs/examples/effect-estimation/ate_estimation.qmd), which covers `ate()` and g-computation, and ideally [Causal Identification](/docs/examples/identification/causal_identification.qmd) for the backdoor/confounding setup used below. You need to be comfortable with what an ATE *is* before it makes sense to ask whether yours is real.
...
|---|---|---|
| [`falsify()`](/docs/examples/identification/dag_falsification.qmd) | Is the DAG consistent with the data? | The graph breaks its conditional-independence promises |
| [`sensitivity()`](/docs/examples/identification/sensitivity.qmd) | Could an unmeasured confounder overturn this? | A plausibly weak confounder reaches the tipping point |
