...
categories: [Causal Inference, Counterfactuals]
jupyter: pathmc
---
...
The `do()` operator cannot answer this because it doesn't condition on Joe's observed data. It averages over all possible individuals, not just those who look like Joe.
...
Joe's exogenous factors are both 0.75 — he has above-average inherent homework effort and above-average inherent exam ability, independent of the encouragement program.
...
```{python}
u_h = JOE_H - a_draws * JOE_X
u_y = JOE_Y - b_draws * JOE_X - c_draws * JOE_H

y_counterfactual = b_draws * JOE_X + c_draws * H_NEW + u_y
```
...
y_cf_hdi = az.hdi(y_counterfactual, hdi_prob=0.94)
...
@fig-counterfactual-posterior shows the full posterior distribution over Joe's counterfactual score. The distribution is narrow because the coefficient $c$ is well-identified with 2000 observations, but wider datasets or weaker identification would produce greater uncertainty.
...
- **Pearl's three-step procedure** — abduction, action, prediction — turns a structural causal model into a counterfactual engine. The abduction step is what distinguishes counterfactuals from interventions: it uses the individual's data to infer their exogenous characteristics.
- **`do()` answers a different question**: the population-level average effect of an intervention. For Joe, `do(H = 2)` predicts a score of $\approx 0.8$ (the average student); the counterfactual predicts $\approx 1.9$ (Joe specifically).
