...
## Causal queries with do()

### Total causal effect of brand spend

The `do()` operator computes the interventional distribution $P(\text{sales} \mid do(\text{brand\_spend} = x))$.
By contrasting two interventions, we get the total causal effect — including both the direct path and the indirect path through search traffic.

```{python}
r_low = model.do(set={"brand_spend": 2.0})
r_high = model.do(set={"brand_spend": 8.0})
brand_ate = r_high - r_low
...
The `do()` result should be close to `0.55 × 6 = 3.3` — the total effect times the intervention size.
