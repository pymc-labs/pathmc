...
::: {.callout-warning}
## `do()` does not yet propagate random slopes

The current `do()` operator accounts for random intercepts but not random slopes.
This means interventional queries reflect only the population-average media effects, not geo-specific coefficients.

For geo-specific counterfactuals, you would need to manually extract the posterior draws for each region's total slope and propagate them yourself.
