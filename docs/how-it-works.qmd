...
3. **Spec + GraphInfo + data → `compile_to_pymc()`**: The AST and graph, combined with the user's data, are compiled into a full `pm.Model`. Design matrices are built via patsy, coefficient priors are emitted, transform parameters are constrained, and likelihoods are wired up equation-by-equation.
...
6. **Post-sampling analysis and causal queries**: Posterior draws drive everything that follows — interventional simulation (`do()`, `ate()`, `cate()`, `prob()`), effect summaries and standardized coefficients, and identification checks (`adjustment_sets()`, `is_identifiable()`, `collider_warnings()`). The DAG topological order is used directly by simulation to propagate interventions parent→child.

## Two graph views (important)

pathmc keeps two related graph representations that serve different jobs:

- **Causal DAG view** (`model.graph()`): domain-level structure used for intervention planning, topological propagation, and identification logic.
- **PyMC factor graph view** (`pm.model_to_graphviz(model.pymc_model)`): statistical/computational structure used for posterior inference.

These are linked, but they are not the same object and should not be interpreted interchangeably.
...
In current pathmc architecture, this is implemented as array-based simulation (xarray/NumPy over posterior draws), not by compiling a second PyMC model or performing PyMC graph surgery.
...
### Estimand semantics note (cross-sectional)

For cross-sectional `do()`, non-intervened exogenous variables are currently initialized to their observed data means. This default is often reasonable for ATE-style summaries, but users should be explicit about estimand definitions when comparing against subgroup-conditional quantities like ATT/ATU.
...
## Scope: observed-variable SEM

pathmc implements **path analysis** — structural equation models over *observed* variables only. It does not support latent variables, factor models, or CFA/SEM measurement models. This is a deliberate design boundary, not an oversight, and understanding where it lives in the architecture helps explain both the current design and what extending it would require.

The core assumption is: **every variable in the DAG corresponds to a column in the data**. This assumption runs through every layer:

| Layer | Where the assumption lives |
|-------|---------------------------|
| **Compiler** | Design matrices are built via patsy from `data` columns. Likelihoods are emitted with `observed=data[var]` for every endogenous variable. |
| **Simulator** | Exogenous variables are initialized from `data.mean()`. Every node in the topological walk is expected to have a data-backed value or a computed one. |
| **Effects** | Standardized effects use `sd(X)` and `sd(Y)` computed from the data. Path effects assume all intermediate variables are observable. |
| **Identification** | Adjustment sets reason over observed variables — d-separation queries assume all nodes could in principle be conditioned on. |

Latent variables would break this assumption in each layer. A latent factor has no data column, so patsy cannot build a design matrix for it; it gets a prior instead of a likelihood; the simulator would need to initialize it from posterior draws rather than data means; and standardization would need posterior SDs instead of data SDs.

The modular architecture means these changes could be isolated per-layer — the parser would gain a new operator (e.g., `=~` for measurement equations), the graph builder would track latent vs observed nodes, and the compiler would emit priors instead of likelihoods for latent factors — but the assumption is woven deeply enough that it would touch every module. Supporting latent variables is a plausible future extension, not a minor patch.
