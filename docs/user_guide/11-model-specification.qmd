...
For levels `north`, `south`, and `west`, the default design columns are `Intercept`, `region[T.south]`, and `region[T.west]`; the first sorted level (`north`) is the reference. Use `C()` when the reference must be explicit:
