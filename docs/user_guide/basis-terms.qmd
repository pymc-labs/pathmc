...
For example, a weekly seasonal pattern with three harmonics is a Fourier basis; the [Fourier seasonality example](/docs/examples/panel-time/fourier_seasonality.html) shows a complete workflow:
...
`hsgp()` uses the same owned-coefficient mechanism but has a kernel-informed prior and a boundary frozen from the fitted data. Its `do()` interventions must remain inside that boundary; see the [HSGP worked example](/docs/examples/foundations/hsgp_1d.html). Fourier has no fitted support boundary, so it remains well-defined outside the observed range.
