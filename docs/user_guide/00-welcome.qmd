...
<img src="logo_light.png" class="gd-hero-logo-light" alt="pathmc logo" style="width: 75%;">
<img src="logo_dark.png" class="gd-hero-logo-dark" alt="pathmc logo" style="width: 75%;">
:::

```{=html}
<style>
.gd-hero-logo-dark { display: none; }
html[data-bs-theme="dark"] .gd-hero-logo-light { display: none; }
html[data-bs-theme="dark"] .gd-hero-logo-dark { display: inline; }
</style>
```
...
Once the model is built, fit it and use its posterior and causal-query methods:
...
pathmc isn't just for textbook examples. The [examples gallery](docs/examples/index.qmd) covers real problems across industries.

::: {.panel-tabset}

### Marketing mix modeling

Estimate channel-level ROI with adstock and saturation transforms that capture carry-over effects and diminishing returns. Panel mode handles geo-level data with hierarchical random effects. [Full example &rarr;](docs/examples/applied-models/mmm.qmd)
...
Assess whether a biomarker is a valid surrogate for clinical outcomes using mediation analysis and the do-operator. Quantify what fraction of the treatment effect flows through the measured pathway. [Full example &rarr;](docs/examples/applied-models/vaccine_surrogates.qmd)
...
Model a multi-stage funnel as a causal chain to find which stage has the highest leverage on paid conversion. Simulate "what if we improve onboarding?" scenarios that respect the full causal structure. [Full example &rarr;](docs/examples/applied-models/saas_funnel.qmd)
...
Estimate true price elasticity from observational data where price and demand are confounded. Panel structure with regional hierarchical effects separates the causal signal from common-cause noise. [Full example &rarr;](docs/examples/applied-models/dynamic_pricing.qmd)
...
- [Overview](user-guide/how-it-works.qmd) — how pathmc works, from spec to causal insight
...
- [Estimation approaches](user-guide/estimation-approaches.qmd) — structural `do()` vs native `adjustment_model()`
- [Predictions, comparisons, and slopes](user-guide/predictions-comparisons-slopes.qmd) — shared interpret API
- [Examples](docs/examples/index.qmd) — worked notebooks across mediation, MMM, panel data, interpret queries, and more
