...
User-facing inputs and outputs use business units: `do(set=)` values are divided by the fitted factor (including per-unit factors) before graph surgery, so `do(set={"tv": 500})` means 500 of the original column, and `predict()`, `do()`, `effects_summary()`, and `effect()` return outcomes and coefficients in business units. Estimation still runs on scaled columns internally; `model.fitted_scaling` exposes the divisors for `simulate()` and manual transforms.
