...
See the [Panel Data Models](../docs/examples/panel-time/panel_data.qmd), [Difference-in-Differences](../docs/examples/panel-time/did.qmd), and [Media Mix Models](../docs/examples/applied-models/mmm.qmd) examples for panel mode in action.
...
The same convention applies to causal estimands and refutation results. `cate()` interprets its intervention `values` and `condition` in business units. `att()` and `atu()` interpret their subgroup and intervention values in business units, converting the subgroup value to the fitted scale before selecting observations; these methods currently support cross-sectional models only. `refute_placebo()` reports both the observed and placebo effects in business units. When `do()` fills an unspecified exogenous variable from the fitted data, it converts each row back to business units before calculating the fill value for the relevant subgroup.

### Convert values explicitly

Use the fitted factors to cross the unit boundary when working directly with scaled data. `to_internal()` divides business-unit values by their fitted factors; `to_business()` reverses the conversion. The `kind` argument identifies the value's role (`"outcome"`, `"regressor"`, or `"coefficient"`); `"elasticity"` and `"derived"` values are dimensionless and remain unchanged. Repeating a conversion in the same direction on its direct result leaves the values unchanged. Scalar results retain that state through arithmetic with Python `int` and `float` values, but `round()` and NumPy operations can return an untagged value. Arithmetic that changes units also retains the tag, so explicitly cross the boundary before using a converted value in a different unit role.

```python
factors = model.fitted_scaling
internal_channels = factors.to_internal(df, kind="regressor")
business_channels = factors.to_business(internal_channels, kind="regressor")
```

For a frame, fitted role metadata selects the columns to convert. If you create `ScalingFactors` manually without roles, select the columns explicitly with `dims={"columns": ("tv",)}`; otherwise frame conversion raises an error rather than guessing. Unnamed individual values need a column in `dims`, for example `dims={"term": "tv"}`. When factors vary by unit, provide row-aligned values and the corresponding data; a single scalar cannot represent different per-unit conversions. Row arrays must end in the fitted row count, and scan-panel arrays must end in `(n_times, n_units)` order. Ambiguous shapes raise an alignment error instead of silently reordering factors.
...
Some quantities are never observed directly at every time step — brand
awareness, true demand, sentiment. pathmc supports **latent variables** in
panel models: declare them with `latent=[...]` and anchor them with a
sparse measurement equation whose rows are `NaN` wherever no survey (or
other measurement) exists:
...
The second line makes the latent state an AR(1) process: each week's
`awareness` is a coefficient times the previous week's state plus process
noise (`families={"awareness": "latent_normal"}`). Because latent variables
have no data column, their initial condition cannot be read off the data —
instead it is *estimated*: an `init_awareness` free parameter (one value per
unit, default prior `Normal(0, 1)`) seeds the recursion. It is the state
*preceding* the first observed period — the first latent value is computed
as `intercept + phi * init_awareness` plus process noise. The default `Normal(0, 1)` prior is not scale-free — override it when the latent is not O(1), otherwise t=0 is pinned near zero and early-period trajectories are biased. Override it like any other prior when you have prior knowledge about the starting level:
...
The result is an `xarray.DataArray` with dimensions `(chain, draw, time,
unit)` and coordinates taken from the model's panel structure. For
stochastic latents it reads the realized state registered under the
variable's own name; for deterministic latents it reads the `mu_{var}`
deterministic.

The same machinery works for forward simulation. `pathmc.simulate()` treats
`init_{var}` as a required parameter alongside coefficients and scales — use
`pathmc.simulate_params_template()` to discover every name and shape:
