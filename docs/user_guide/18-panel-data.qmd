...
Use multiple names to pool over a dimension combination, for example `"tv": {"coefficient": ("geo", "brand")}` creates `mu_tv_geo_brand[geo, brand]`. If the observed panel omits some combinations, those unused hyperprior cells remain prior-only; use a rectangular panel when every cell should be informed by data.
...
`do(set=)` values are in business units and are divided by the fitted factor (including per-unit factors) before graph surgery, so `do(set={"tv": 500})` means 500 of the original column. `predict()` / `do()` *outputs* remain in scaled units when the target role was scaled; multiply by `model.fitted_scaling` if you need business-unit outcomes.
