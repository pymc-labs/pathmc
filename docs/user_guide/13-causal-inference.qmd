...
Each permutation triggers a full MCMC re-fit, so the cost scales with `n_permutations` (4 is the practical minimum for identifying `tau_het`). Pass `sample_kwargs={...}` to control the sampler and `random_seed=` for reproducibility. Panel models are not yet supported.
